...
# Extract the wasm file from the first build
docker run --rm -v "$tmpdir:/output" canisters cp /cycles-burner-canister.wasm.gz /output/cycles-burner-canister.wasm.gz

# Calculate the SHA256 sum for the first build
echo "Calculating SHA256 sums (1st build)..."
sha256sum1=$(sha256sum "$tmpdir/cycles-burner-canister.wasm.gz")
...
sha256sum2=$(sha256sum "$tmpdir/cycles-burner-canister.wasm.gz")
