...
set_correct_replica_and_canister_sandbox(){
    chmod +x replica canister_sandbox ic-https-outcalls-adapter
    PATH_TO_DFX_CACHE="$HOME/.cache/dfinity/versions/$(dfx --version | awk '{ print $2 }')"
    rm -f "$PATH_TO_DFX_CACHE/replica" "$PATH_TO_DFX_CACHE/canister_sandbox" "$PATH_TO_DFX_CACHE/ic-https-outcalls-adapter" 
